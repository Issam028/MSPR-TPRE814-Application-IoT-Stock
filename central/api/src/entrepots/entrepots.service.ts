...
export class EntrepotsService {
  ...
  async findAll(country: string) {
    const { data } = await firstValueFrom(
      this.httpService.get(`${this.getBaseUrl(country)}/entrepots`),
    );
    return data;
  }
  ...
  async create(country: string, body: any) {
    const { data } = await firstValueFrom(
      this.httpService.post(`${this.getBaseUrl(country)}/entrepots`, body),
    );
    return data;
  }
  ...
  async update(country: string, id: string, body: any) {
    const { data } = await firstValueFrom(
      this.httpService.put(`${this.getBaseUrl(country)}/entrepots/${id}`, body),
    );
    return data;
  }
  ...
  async remove(country: string, id: string) {
    await firstValueFrom(
      this.httpService.delete(`${this.getBaseUrl(country)}/entrepots/${id}`),
    );
  }
}
